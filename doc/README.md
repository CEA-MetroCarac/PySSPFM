...
<p align="justify" width="100%">
Calibration is indispensable for obtaining quantitative measurements. In the measurement data sheet, values can be provided to quantify the measured amplitude, including tip sensitivity (nm/V) and spring constant (N/m), which can be obtained from the manufacturer or through pre-measurement calibration. Additionally, a pre-measurement calibration can be used to determine the phase offset. All amplitude and phase values are calibrated with the result in the scripts <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/data_processing/seg_to_loop_s1.py">executable source code</a></code> and <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/seg_to_loop/analysis.py">seg_to_loop/analysis</a></code> with the function <code>zi_calib</code>.
</p>
...
<p align="justify" width="100%">
L'ensemble des scripts <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/tree/main/PySSPFM/utils/core">core</a></code> contient des fonctions relativement génériques sur lesquels se basent la majorité des scripts de l'application PySSPFM, plus spécifiques au traitement des mesures SSPFM. L'ensemble des scripts est composé de:
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/basic_func.py">basic_func.py</a></code> qui contient un ensemble de modèles algébriques sur lesquels reposent les fits réalisés
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/curve_hysteresis.py">curve_hysteresis.py</a></code> qui permet l'initialisation et le traitement de l'objet <code>Hysteresis</code> (voir la section: VI.3) - Hysteresis and properties de la documentation)
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/figure.py">figure.py</a></code> qui permet de générer des figures selon un même style. Des graphiques, histogrammes ou cartographies peuvent ainsi être géénérés, respectveiement via les fonctions <code>plot_graph</code>, <code>plot_hist</code> et <code>plot_map</code>. La fonction <code>print_plots</code> permet de gérer de manière avancée l'affichage et l'enregistrement des figures.
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/curve_hysteresis.py">curve_hysteresis.py</a></code> permet la réalisation de l'ensemble des fits (sauf le fit de l'hystérésis) en se basant sur la librairie <a href="https://pypi.org/project/lmfit/">lmfit</a>. Elle contient une classe mère <code>CurveFit</code> et 3 classes filles <code>GaussianPeakFit</code>, <code>ShoPeakFit</code>, <code>ShoPhaseFit</code> se basant respectivement sur la classe mère pour la réalisation de fit gaussien, sho et de phase sho (arctangent). La classe mère contient un ensemble de méthodes communes aux classes filles, à savoir les méthodes <code>eval</code> pour evaluate the fitted peak at given x-values, <code>fit</code>, <code>plot</code> etc. Les classes filles appellent la classes mère au moment de l'initialisation et permettent également d'initialiser les paramètres de fit, via un guess propres au modèle fité.
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/iterable.py">iterable.py</a></code> pour le traitement d'itérables.
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/noise.py">noise.py</a></code> pour la gestion du bruit. <code>filter_mean</code> est une fonction de filtre de moyennage, d'ordre choisi, qui permet de réduire le bruit des mesures. La fonction <code>noise</code> permet de générer du bruit d'une certaine amplitude (beaucoup utilisé pour les exemples et tests, afin de reconstituer des données les plus réalistes possibles), selon 3 modèles de répartition : <code>unfirom</code>, <code>normal</code> et <code>laplace</code>.
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/peak.py">peak.py</a></code> contient un ensemble de fonction pour le traitement de pics. <code>detect_peak</code> et <code>find_peaks</code> permettent de trouver automatiquement des pics dans un tableau de valeur, en se basant sur la fonction <code>find_peaks</code> de la librairie scipy.signal. Des fonction de guess de composante de bruit (composante affine avec la fonction <code>guess_affine</code> et constante avec la fonction <code>guess_bckgnd</code>) et de détermination de largeur de pic avec la fonction <code>width_peak</code>.
- <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/core/signal.py">signal.py</a></code> contient deux fonctions: <code>line_reg</code> pour faire de la régression linéaire et <code>interpolate</code> pour de l'interpolation 1D.
</p>
