...
<p align="justify" width="100%">
The initial step entails the extraction of properties generated after the second processing stage, residing within the folder <code>'txt_ferro_meas'</code>, accomplished by the <code>extract_measures</code> function in the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/hyst_to_map/file.py">utils/hyst_to_map/file</a></code>. The property selected by the user through the <code>'meas key'</code> parameter, serving as a sorting reference, is then singled out among all others. The extraction of processing and measurement parameters from the file <code>'results/parameters.txt'</code> is carried out using the <code>generate_pars</code> function from the script. The measurement files are then sorted under two scenarios: <br>
&#8226 If the <code>list_pixels</code> parameter is <code>None</code>: based on the value of the user-chosen reference property. <br>
&#8226 If the <code>list_pixels</code> parameter is <code>[]</code>: according to the file index. If the list is empty, all pixels are considered; otherwise, only the indices listed are considered. <br>
Then, for each of the files:
    <ul>
        <li>If <code>mode = 'on'</code> or <code>mode = 'off'</code>:</li>
            <ul>
                <li>An analysis of the entire file selection is performed using the <code>mean_loop</code> tool (the main function <code>main_mean_loop</code> from the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/mean_loop.py">toolbox/mean_loop.py</a></code> is called). The average hysteresis and properties are then determined.</li>
                <li>The nanoloop of the corresponding file (located in the 'txt_loops' folder) is extracted with the <code>extract_loop</code> function from <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/nanoloop/file">utils/nanoloop/file</a></code>, and the main function <code>main_loop_file_reader</code> from the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/loop_file_reader.py">toolbox/loop_file_reader</a></code> is utilized to create figures in the form of nanoloops.</li>
                ...
                <li>A comparative figure between the average hysteresis and that of the respective file is constructed with the <code>construct_hyst_comp</code> function from the script.</li>
            </ul>
        <li>If <code>mode = 'coupled'</code>:</li>
            <ul>
                <li>For each mode (<code>'on'</code> and <code>'off'</code>): The nanoloop of the corresponding file (located in the 'txt_loops' folder) is extracted with the <code>extract_loop</code> function from <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/nanoloop/file">utils/nanoloop/file</a></code>, and the <code>best_loop</code> is extracted with the <code>single_script</code> function from the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/data_processing/hyst_to_map_s2.py">data_processing/hyst_to_map_s2.py</a></code>. The main function <code>main_loop_file_reader</code> from the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/loop_file_reader.py">toolbox/loop_file_reader</a></code> is employed to create figures in the form of nanoloops.</li>
                <li>An analysis of the entire file selection is performed using the <code>mean_loop</code> tool (the main function <code>main_mean_loop</code> from the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/mean_loop.py">toolbox/mean_loop.py</a></code> is called). The differential component and average properties are then determined.</li>
                <li>The <code>differential_analysis</code> function in <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/hyst_to_map/electrostatic.py">utils/hyst_to_map/electrostatic.py</a></code> is employed to
 ...
 conduct the differential analysis for the respective file.</li>
        ...
        <li>The mapping of the reference property is constructed using the <code>plot_and_save_image</code> function, with the pixel corresponding to the highlighted file. A histogram of the reference property is generated, and the value of the latter for the respective file is highlighted.</li>
    </ul>
</p>

<p align="center" width="100%">
La première étape consiste en l'extraction des propriétés généré après la deuxième étape de traitement, contenues dans le dossier <code>'txt_ferro_meas'</code> with <code>extract_measures</code> function of the script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/hyst_to_map/file.py">utils/hyst_to_map/file</a></code>. La propriété choisie par l'utilisateur via le paramètre <code>'meas key'</code>, utilisée comme référence de trie est alors sélectionnée parmis toutes les autres. L'extraction des paramètres de traitements et de mesures contenus dans le fichier <code>'results/parameters.txt'</code> est faite avec la fonction <code>generate_pars</code> du script.  Les fichiers de mesures sont alors triés, selon 2 cas de figure :
- si le paramètre <code>list_pixels is None</code>: en fonction de la valeur de la propriété de référence choisie par l'utilisateur
- si le paramètre  <code>list_pixels = []</code> : en fonction de l'indice du fichier. Si la liste est vide, l'ensemble des pixels est considéré, sinon, seul les indices renseignés dans la liste sont considérés.
Puis pour chacun des fichiers:
- si <code>mode = 'on'</code> ou <code>mode = 'off'</code> :
-     - Une analyse de l'ensemble de la sélection de fichier est effectuée avec le tool </code>mean_loop<code> (la fonction principale main_mean_loop du script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/mean_loop.py">toolbox/mean_loop.py</a></code> est appelée). L'hysterésis et propriétés moyennes sont alors déterminées.
-     - La nanoloop du fichier correspondant (contenu dans le dossier 'txt_loops') est extraite with the <code>extract_loop</code> function from the <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/nanoloop/file">utils/nanoloop/file</a></code> et la fonction principale <code>main_loop_file_reader</code> du script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/loop_file_reader.py">toolbox/loop_file_reader</a></code> permet de créer les figures sous forme de nanoloops.
-     - La fonction <code>single_analysis</code> du script est appelée. Elle permet d'extraire la <code>best_loop</code>, de construire et analyser l'objet <code>Hysteresis</code> (fit et extraction des propriétés).
-     - la figure comparative entre l'hystérésis moyenne et celle du fichier en question est construite avec la fonction <code>construct_hyst_comp</code> du script
- si <code>mode = 'coupled'</code> :
-     - pour chaque mode (<code>'on'</code> et <code>'off'</code>):  La nanoloop du fichier correspondant (contenu dans le dossier 'txt_loops') est extraite with the <code>extract_loop</code> function from the <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/nanoloop/file">utils/nanoloop/file</a></code>, la <code>best_loop</code> est extraite avec La fonction <code>single_script</code> du script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/data_processing/hyst_to_map_s2.py">data_processing/hyst_to_map_s2.py</a></code>. La fonction principale <code>main_loop_file_reader</code> du script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/loop_file_reader.py">toolbox/loop_file_reader</a></code> permet de créer les figures sous forme de nanoloops.
-     - Une analyse de l'ensemble de la sélection de fichier est effectuée avec le tool </code>mean_loop<code> (la fonction principale main_mean_loop du script <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/toolbox/mean_loop.py">toolbox/mean_loop.py</a></code> est appelée). La composate différentielle et les propriétés moyennes sont alors déterminées.
-     - The <code>differential_analysis</code> function in <code><a href="https://github.com/CEA-MetroCarac/PySSPFM/blob/main/PySSPFM/utils/hyst_to_map/electrostatic.py">utils/hyst_to_map/electrostatic.py</a></code> permet d'effectuer l'analyse différentielle pour le fichier en question.
    - Les composantes différentielles (moyennes et du fichier en question) sont déterminées au cours des étapes d'analyse correspondantes
- La cartographies de la propriété de réference est construite avec la fonction <code>plot_and_save_image</code>, avec le pixel correspondant au fichier mis en évidence. Un histogramme de la propriété de référence est déterminé, et la valeur de cette dernière pour le fichier en question est mise en évidence.
</p>
