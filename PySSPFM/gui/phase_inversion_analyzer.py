"""
--> Executable Script
Graphical interface for automatic determination of phase inversion for a list of
raw sspfm measurement file
(run phase_inversion_analyzer.main_phase_inversion_analyzer)
"""
...
from PySSPFM.toolbox.phase_inversion_analyzer import \
    main_phase_inversion_analyzer as main_script
...
def main(parent=None):
    ...
    title = "Phase inversion analyzer"
    ...
    def launch():
        ...
        if user_parameters['save']:
            create_json_res(user_parameters, user_parameters['dir path out'],
                            fname="phase_inversion_analyzer_params.json",
    ...
    # Window title: Phase inversion analyzer
    ...
    strg = "- Name: range_file\n" \
           ...
           "of indices of raw measurement files to be used for phase " \
           "inversion analysis.\n" \
    ...
    label_phase = ttk.Label(scrollable_frame, text="Phase offset",
                            font=("Helvetica", 14))
    ...
    label_phase_file = ttk.Label(scrollable_frame,
                                 text="Phase offset file path (in) (*):")
    ...
    entry_phase_file = ttk.Entry(scrollable_frame,
                                 textvariable=phase_file_path_var)
    ...
    browse_button_phase_file = ttk.Button(scrollable_frame, text="Select",
                                          command=browse_file_phase)
    row = grid_item(browse_button_phase_file, row, column=2)

    # Value
    label_value_offset = ttk.Label(scrollable_frame, text="Offset:")
    ...
    entry_fwd = ttk.Entry(scrollable_frame, textvariable=pha_offset_var)
    entry_fwd.grid(row=9, column=1, sticky="ew")
    row = grid_item(entry_fwd, row, column=1, sticky="ew")
    strg = "- Name: offset\n" \
